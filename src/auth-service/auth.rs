...
impl Auth for AuthService {
    async fn sign_in(
        ...
        let result: Option<String> = self.users_service.lock().expect("failed to lock thread sign-in result").get_user_uuid(req.username, req.password); // Get user's uuid from `users_service`. Panic if the lock is poisoned.
        ...
        let user_uuid: String = match result {
            Some(uuid) => uuid,
            None => return Ok(Response::new(SignInResponse { status_code: StatusCode::Failure.into(), user_uuid: "".to_owned(), session_token: "".to_owned() }))
        };

        let session_token: String = self.sessions_service.lock().expect("failed to lock thread sign-in session").create_session(&user_uuid); // Create new session using `sessions_service`. Panic if the lock is poisoned.
    ...
    async fn sign_up(
        ...
        let result: Result<(), String> = self.users_service.lock().expect("failed to lock thread sign-up").create_user(req.username, req.password); // Create a new user through `users_service`. Panic if the lock is poisoned.
        ...
        match result {
            ...
            Err(_) => {
                Ok(Response::new(SignUpResponse{ status_code: StatusCode::Failure.into() }))
            }
        }
    }
    ...
    async fn sign_out(
        ...
        self.sessions_service.lock().expect("failed to lock thread sign-out").delete_session(&req.session_token);
